...
mask_fil=np.zeros(np.shape(mask))
...
mask_sht=np.zeros(np.shape(mask))
mask_sht_indx=np.where(mask==1)
mask_sht[mask_sht_indx]=1
mask_sht=(mask_sht !=1)
recon_vecs_x[mask_sht]=0
recon_vecs_y[mask_sht]=0
recon_vecs_z[mask_sht]=0
'''
...
mlab.pipeline.vectors(pltt,scale_factor=3,mask_points=800,line_width=0.6)#mask_points=2 This is for the eigenvectors lSS
