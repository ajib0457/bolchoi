...
f=h5py.File("/import/oth3/ajib0457/bolchoi_z0/investigation/my_denbox/DTFE_cutout_%s_%s_%s_%s_%s_%s_grid%d_smth%sMpc_eig_xyz_mask_Vxyz_Lxyz_Vm_Vr_resid.h5" %(box_sz_x_min,box_sz_x_max,box_sz_y_min,box_sz_y_max,box_sz_z_min,box_sz_z_max,grid_nodes,round(std_dev_phys,2)), 'r')
for i in arrys:
    diction[i]=f['/%s'%i][:]
f.close()
...
manifest=['clusters','filaments_vecs']#make sure to put 'cluster' before 'filament' in this list otherwise 'filament' will supersede 'cluster' iso
...
#        mlab.axes(ylabel='y[Mpc/h]',xlabel='x[Mpc/h]',zlabel='z[Mpc/h]')
    ...
    if i=='filaments_vecs':
        ...
        pltt_fil=mlab.pipeline.vector_field(a_x_new,a_y_new,a_z_new)#optional: vmin=0.6 and vmax=0.4
        mlab.pipeline.vectors(pltt_fil,scale_factor=3,mask_points=60,line_width=0.6,color=(1,1,1))#mask_points=2 This is for the eigenvectors lSS
    ...
    if i=='voids':
        ...
        mlab.pipeline.iso_surface(magnitude_vod, contours=[1.9, 0.5],color=(0.06666, 0.06666, 0.1804),opacity=0.9)#this is the isosurfaces plotter. You can simply delete these two lines to plot a vector field
...
#                print(x,y-box_sz_y_min,z,diction['resid'][j,6],diction['resid'][j,7],diction['resid'][j,8])
