...
for part in range(tot_parts):
    ...
    f=h5py.File("/scratch/GAMNSCM2/bolchoi_z0/correl/DTFE/files/output_files/eigvecs_smthd/fil_recon_vecs_DTFE_gd%d_smth%sMpc_%d_vecsmthd_%sMpc/h.h5" %(grid_nodes,round(std_dev_phys,3),part,std_dev_phys_vec), 'w')
    recon_vecs_x[nrows_in:nrows_fn]=f['/group%d/x_smthd'%part][:]
    recon_vecs_y[nrows_in:nrows_fn]=f['/group%d/y_smthd'%part][:]
    recon_vecs_z[nrows_in:nrows_fn]=f['/group%d/z_smthd'%part][:]
    f.close()
...
f=h5py.File('/scratch/GAMNSCM2/bolchoi_z0/correl/DTFE/files/output_files/dotproduct/spin_lss/smthd_eigvecs/DTFE_grid%d_spin_store_fil_Log%s-%s_smth%sMpc_%sbins_smthdvecs_%sMpch.h5'%(grid_nodes,round(low_int_mass,2),round(hi_int_mass,2),round(std_dev_phys,3),tot_mass_bins,std_dev_phys_vec),'w')     
